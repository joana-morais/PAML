#!/bin/bash
#SBATCH -J HOX_MAFFT_FASTAtoPHYL
...
#SBATCH -o /scratch/joanamorais/jobs/01_OUTandERR/HOX_MAFFT_FASTAtoPHYL.out
#SBATCH -e /scratch/joanamorais/jobs/01_OUTandERR/HOX_MAFFT_FASTAtoPHYL.err
...
process_gene() {
    ...
    local input_file="$gene_dir/${gene_name}_MAFFT_Pal2Nal.fasta"
    ...
    echo "Running FASTAtoPHYL.pl for $gene_name MAFFT file..."
    ...
    if [ $? -eq 0 ]; then
        ...
        local temp_output_file="${gene_name}_MAFFT_Pal2Nal.phy"

        if [ -f "$temp_output_file" ]; then
            # Move the file to the final destination with the desired naming
            local final_output_file="$output_dir/${gene_name}_MAFFT_Pal2Nal_one_line.fa.phy"
            mv "$temp_output_file" "$final_output_file"

            echo "Successfully converted $gene_name MAFFT file to PHYLIP format"
            echo "Created $final_output_file ($(wc -l < "$final_output_file") lines)"
        else
            echo "Warning: Expected MAFFT output file not found in temp directory"
            # List files in temp directory for debugging
            echo "Files in temp directory:"
            ls -la
        fi
    else
        echo "Error converting $gene_name MAFFT file to PHYLIP format"
    fi
...
if [ "$TEST_MODE" = true ]; then
    echo "TEST MODE: Processing only gene $TEST_GENE MAFFT file with FASTAtoPHYL.pl"
    ...
    echo "FULL MODE: Processing all HOX genes MAFFT files with FASTAtoPHYL.pl"
    ...
    echo "All HOX genes MAFFT files processed. Check the output directories for results."
fi